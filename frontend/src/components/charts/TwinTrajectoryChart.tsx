              ...
              <div className="rounded-lg border border-surface-200 bg-white px-3 py-2 shadow-lg text-xs dark:border-surface-700 dark:bg-surface-800">
                ...
                <div className="mt-1 space-y-0.5">
                  <p>
                    ...
                    Biomass: <span className="font-bold">{formatNumber(item.biomass, 3)}</span> kg
                  </p>
                  <p>
                    <span className="inline-block w-2 h-2 rounded-full bg-amber-500 mr-1.5" />
                    Substrate: <span className="font-bold">{formatNumber(item.substrate, 3)}</span> kg
                  </p>
                  <p>
                    <span className="inline-block w-2 h-2 rounded-full bg-purple-500 mr-1.5" />
                    Growth: <span className="font-bold">{formatNumber(item.growth_rate, 5)}</span> 1/h
                  </p>
                  <p className="text-surface-400">
                    T: {formatNumber(item.temperature, 1)}��C �� M: {formatNumber(item.moisture, 1)}%
                  </p>
                </div>
              </div>
            );
