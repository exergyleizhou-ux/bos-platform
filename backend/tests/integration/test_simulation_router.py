...
class TestMonteCarloSimulation:
    ...
    async def test_simulation_basic(self, client: AsyncClient, admin_headers):
        ...
        batch_resp = await client.post(
            "/api/v1/batches",
            json={"batch_id": "MC-TEST-001", "dm_in": 10.0, "dm_out": 2.3},
    ...
    async def test_simulation_large_n(self, client: AsyncClient, admin_headers):
        ...
        batch_resp = await client.post(
            "/api/v1/batches",
            json={"batch_id": "MC-TEST-002", "dm_in": 10.0, "dm_out": 2.3},
            headers=admin_headers,
        )
        batch_id = batch_resp.json()["id"]
    ...
    async def test_simulation_percentiles(self, client: AsyncClient, admin_headers):
        ...
        batch_resp = await client.post(
            "/api/v1/batches",
            json={"batch_id": "MC-TEST-003", "dm_in": 10.0, "dm_out": 2.3},
            headers=admin_headers,
        )
        batch_id = batch_resp.json()["id"]
    ...
    async def test_simulation_ci(self, client: AsyncClient, admin_headers):
        """Confidence interval bounds should contain the mean."""
        batch_resp = await client.post(
            "/api/v1/batches",
            json={"batch_id": "MC-TEST-004", "dm_in": 10.0, "dm_out": 2.3},
            headers=admin_headers,
        )
        batch_id = batch_resp.json()["id"]
