...
class TestForecast:
    ...
    def test_sma_forecast(self):
        values = [0.15, 0.14, 0.16, 0.13, 0.17, 0.12, 0.15, 0.14]
        result = run_forecast(
            ForecastInput(
                values=values,
                method="sma",
    ...
    def test_ewma_forecast(self):
        values = [0.20, 0.18, 0.19, 0.17, 0.16, 0.15, 0.14, 0.13]
        result = run_forecast(
            ForecastInput(
                values=values,
                method="ewma",
        ...
        # Downward trend �� forecast should be below last value
        assert result.forecast[0] <= values[-1] + 0.02
    ...
    def test_ci_bounds_order(self):
        values = [0.15, 0.14, 0.16, 0.13, 0.17, 0.12]
        result = run_forecast(
            ForecastInput(values=values, method="sma", horizon=3, window=3)
        )
    ...
    def test_mape_and_rmse(self):
        values = [0.15, 0.14, 0.16, 0.13, 0.17, 0.12, 0.15, 0.14]
        result = run_forecast(
            ForecastInput(values=values, method="sma", horizon=2, window=3)
        )
