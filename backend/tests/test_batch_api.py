...
class TestBatchCreate:
    ...
    async def test_create_batch_missing_required(
    ...
    async def test_create_batch_unauthorized(
        self, client: AsyncClient, sample_batch_payload: dict
    ):
        resp = await client.post("/api/v1/batches", json=sample_batch_payload)
        assert resp.status_code == 401

    async def test_create_batch_negative_dm(
        self, client: AsyncClient, auth_headers: dict
    ):
        payload = {
            "batch_id": "BSF-NEG",
        ...
        resp = await client.post(
            "/api/v1/batches", json=payload, headers=auth_headers
        )
        assert resp.status_code == 422
...
class TestBatchDetail:
    ...
    async def test_get_batch_not_found(
        ...
        resp = await client.get("/api/v1/batches/99999", headers=auth_headers)
...
class TestBatchUpdate:
    ...
    async def test_update_batch_success(
        ...
        resp = await client.patch(
            ...
            json={"notes": "Updated notes", "temperature": 30.0},
            headers=auth_headers,
        )
        assert resp.status_code == 200
        data = resp.json()
