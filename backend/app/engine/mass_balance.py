...
def reconcile_mass_balance(inp: MassBalanceInput) -> MassBalanceResult:
    ...
    result.raw_balance_error = round(raw_error, 6)
    ...
    for i, label in enumerate(labels):
        ...
        if y[i] != 0:
            result.adjustment_pct[label] = round(abs(adj) / abs(y[i]) * 100, 2)
        else:
            result.adjustment_pct[label] = 0.0

    # Check reconciled balance
    rec_total_out = y_rec[1] + y_rec[2] + y_rec[3] + y_rec[4]
    result.reconciled_balance_error = round(float(y_rec[0] - rec_total_out), 8)
    ...
    if np.any(sigmas > 0):
        ...
        result.chi_squared_acceptable = chi_sq <= chi_crit

    # Warnings
    for label in labels:
        if result.adjustment_pct.get(label, 0) > inp.max_adjustment_pct:
            result.warnings.append(
                f"Large adjustment on {label}: {result.adjustment_pct[label]:.1f}% "
