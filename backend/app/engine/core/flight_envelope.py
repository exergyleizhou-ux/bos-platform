"""
BOS Pipeline v9.0 flight envelope engine.
...
def classify_zone(
    ...
    """Classify a parameter value into green, yellow, or red."""
...
def deviation_from_optimal(value: float, optimal_min: float, optimal_max: float) -> float:
    """Calculate percentage deviation from the optimal center."""
...
def check_flight_envelope(inp: FlightEnvelopeInput) -> FlightEnvelopeResult:
    """Perform flight envelope checks for all provided parameters."""
    ...
    if inp.temperature is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"Temperature {inp.temperature} degC is outside optimal range ({opt_min}-{opt_max} degC)"
            warnings.append(msg)
        elif zone == "red":
            if inp.temperature <= sp.temp_lethal_low:
                msg = (
                    f"LETHAL: Temperature {inp.temperature} degC is at or below lethal threshold "
                    f"({sp.temp_lethal_low} degC)"
                )
            elif inp.temperature >= sp.temp_lethal_high:
                msg = (
                    f"LETHAL: Temperature {inp.temperature} degC is at or above lethal threshold "
                    f"({sp.temp_lethal_high} degC)"
                )
            else:
                msg = f"ALARM: Temperature {inp.temperature} degC is outside safe range ({safe_min}-{safe_max} degC)"
            alarms.append(msg)
    ...
    if inp.moisture is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"Moisture {inp.moisture}% outside optimal ({opt_min}-{opt_max}%)"
            warnings.append(msg)
        elif zone == "red":
            msg = f"ALARM: Moisture {inp.moisture}% outside safe range ({safe_min}-{safe_max}%)"
            alarms.append(msg)
    ...
    if inp.feed_rate is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"Feed rate {inp.feed_rate} g/larva/day outside optimal ({opt_min:.3f}-{opt_max:.3f})"
            warnings.append(msg)
        elif zone == "red":
    ...
    if inp.density is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"Density {inp.density} larvae/m2 outside optimal ({opt_min:.0f}-{opt_max:.0f})"
            warnings.append(msg)
        elif zone == "red":
    ...
    if inp.ph is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"pH {inp.ph} outside optimal ({opt_min}-{opt_max})"
            warnings.append(msg)
        elif zone == "red":
            msg = f"ALARM: pH {inp.ph} outside safe range ({safe_min}-{safe_max})"
            alarms.append(msg)
    ...
    if inp.o2_level is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"O2 level {inp.o2_level}% below optimal"
            warnings.append(msg)
        elif zone == "red":
            msg = f"ALARM: O2 level {inp.o2_level}% critically low"
            alarms.append(msg)
    ...
    if inp.co2_level is not None:
        ...
        msg = ""
        if zone == "yellow":
            msg = f"CO2 level {inp.co2_level}% elevated"
            warnings.append(msg)
        elif zone == "red":
            msg = f"ALARM: CO2 level {inp.co2_level}% dangerously high"
            alarms.append(msg)
