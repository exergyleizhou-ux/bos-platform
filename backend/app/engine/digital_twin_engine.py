...
The dynamics follow simplified Monod kinetics:
  dB/dt = ��_max * S/(K_s + S) * B - k_death * B
  dS/dt = -1/Y * ��_max * S/(K_s + S) * B + feed_rate
  dT/dt = (T_env - T)/��_T + Q_met/(m*c_p) + Q_heat
  dM/dt = (M_env - M)/��_M - evap_rate * ventilation
  dN/dt = -k_n * ��_max * S/(K_s + S) * B
"""
...
# Legacy alias retained for older unit/integration tests.
...
def step_twin(state: TwinState, config: TwinConfig, dt: float = 1.0) -> TwinState:
    """Legacy single-step helper used by older tests."""
    return predict_step(state=state, inputs={}, params=config, dt=dt).state
...
def run_twin_simulation(
    ...
    """Legacy trajectory helper returning states (initial + n_steps)."""
    if n_steps < 0:
        raise ValueError("n_steps must be >= 0")
    states: List[TwinState] = [initial_state]
    current = initial_state
    for _ in range(n_steps):
        current = step_twin(current, config, dt=dt)
