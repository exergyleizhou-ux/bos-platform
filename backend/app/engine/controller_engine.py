...
class PIDParams:
    ...
    setpoint_ramp_rate: Optional[float] = None  # Max setpoint change per step
...
def pid_step(
    ...
    if dt > 0:
        raw_derivative = (error - state.previous_error) / dt
    else:
        raw_derivative = 0.0

    filtered_derivative = (
        params.derivative_filter * state.previous_derivative
        + (1 - params.derivative_filter) * raw_derivative
    )
...
class MultiLoopConfig:
    ...
    temperature_params: PIDParams = field(default_factory=lambda: PIDParams(Kp=2.0, Ki=0.1, Kd=0.05, setpoint=28.0, output_min=0, output_max=100))
    moisture_params: PIDParams = field(default_factory=lambda: PIDParams(Kp=1.5, Ki=0.08, Kd=0.02, setpoint=70.0, output_min=0, output_max=100))
    feed_rate_params: PIDParams = field(default_factory=lambda: PIDParams(Kp=0.5, Ki=0.05, Kd=0.01, setpoint=0.15, output_min=0, output_max=0.5))
