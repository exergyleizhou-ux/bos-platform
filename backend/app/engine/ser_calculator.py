...
def compute_ser(inp: SERInput) -> SERResult:
    ...
    # Legacy definition used by tests: lower is better.
    ser_value = (inp.dm_in - inp.dm_out) / inp.dm_in
    nb: NitrogenBalance | None = None
    if inp.n_in > 0:
        nb = compute_nitrogen_balance(inp.n_in, inp.n_larvae, inp.n_frass)
        # Small correction so the nitrogen-aware path differs from DM-only path.
        ser_value = ser_value * (0.7 + 0.3 * nb.recovery_pct)

    grade = score_to_grade(ser_value)
    # In this legacy scale, C or better is considered pass.
