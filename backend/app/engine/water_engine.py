...
class WaterInput:
    ...
    # Direct water use (L)
    process_water: Optional[float] = None  # If measured directly
    cleaning_water: Optional[float] = None
    cooling_water: Optional[float] = None
    water_direct_litres: Optional[float] = None  # Legacy direct-water field
    water_recycled_litres: float = 0.0  # Legacy recycled-water credit
    water_scarcity_index: float = 0.0  # Legacy WSI scalar

    # Energy for cooling (kWh)
    cooling_energy_kwh: float = 0.0

    # Substrate origin
    substrate_wf_blue: Optional[float] = None  # Override L/kg DM
    substrate_wf_green: Optional[float] = None
    substrate_wf_grey: Optional[float] = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.process_water is None and self.water_direct_litres is not None:
            self.process_water = self.water_direct_litres
...
def compute_water_footprint(inp: WaterInput) -> WaterResult:
    ...
    if inp.water_recycled_litres > 0:
        result.blue_process = round(max(result.blue_process - inp.water_recycled_litres, 0.0), 2)
