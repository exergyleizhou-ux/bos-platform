...
class ForecastInput:
    ...
    seasonal_period: Optional[int] = None  # For seasonal decomposition
...
def forecast_timeseries(inp: ForecastInput) -> ForecastResult:
    ...
    result.ci_lower = [round(float(f - w), 6) for f, w in zip(forecast, ci_width)]
    result.ci_upper = [round(float(f + w), 6) for f, w in zip(forecast, ci_width)]
