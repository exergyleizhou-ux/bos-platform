...
class SERInput:
    ...
    def validate(self) -> List[str]:
        ...
        if self.dm_in > 0 and self.dm_out / self.dm_in > 1:
...
PASS_THRESHOLD = SER_PASS_THRESHOLD
...
def generate_recommendations(inp: SERInput, result: SERResult) -> List[str]:
    ...
    if result.ser_value < 0.10:
        recs.append("CRITICAL: SER below 0.10 indicates severe conversion issues. Review substrate quality and larval health.")
    ...
    if result.fat_balance is not None:
        if result.fat_balance > 2.0:
            recs.append("Very high fat conversion. BSF larvae are accumulating lipids well �� consider lipid extraction.")
...
def compute_ser(inp: SERInput) -> SERResult:
    ...
    # Keep the simplified legacy-compatible expectation used by tests.
