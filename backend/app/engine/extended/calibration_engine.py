...
def matern52_kernel(X1: NDArray, X2: NDArray, length_scale: float, variance: float) -> NDArray:
    """Matérn 5/2 kernel — smoother than RBF, more realistic for physical processes."""
...
class GPInput:
    ...
    X_predict: list[list[float]] | None = None  # Prediction points
    kernel: str = "matern52"  # "rbf" or "matern52"
    noise_variance: float = 0.01  # Observation noise σ2_n
...
def fit_gp(inp: GPInput) -> GPResult:
    ...
    result.y_pred_ci_lower = [round(float(m - 1.96 * s), 6) for m, s in zip(pred_mean, pred_std, strict=False)]
    result.y_pred_ci_upper = [round(float(m + 1.96 * s), 6) for m, s in zip(pred_mean, pred_std, strict=False)]
