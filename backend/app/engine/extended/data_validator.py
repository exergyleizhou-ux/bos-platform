...
def validate_batch_data(inp: ValidationInput) -> ValidationResult:
    ...
    if (
        dm_in is not None
        and dm_out is not None
        and isinstance(dm_in, int | float)
        and isinstance(dm_out, int | float)
        and dm_out > dm_in
    ):
        result.issues.append(
            ValidationIssue(
    ...
    if all(v is not None and isinstance(v, int | float) for v in [n_in, n_larvae, n_frass]) and n_in > 0:
            ...
            if temp < sp.temp_lethal_low:
                ...
                result.issues.append(
                    ValidationIssue(
                        ...
                        message=f"Temperature {temp}degC outside normal range ({sp.temp_min}-{sp.temp_max}degC) for {sp.common_name}",
                        value=temp,
                    )
                )

        moisture = data.get("moisture")
        if moisture is not None and isinstance(moisture, int | float) and (
            moisture < sp.moisture_min or moisture > sp.moisture_max
        ):
            result.issues.append(
                    ValidationIssue(
                        field="moisture",
                        ...
                        message=f"Moisture {moisture}% outside range ({sp.moisture_min}-{sp.moisture_max}%) for {sp.common_name}",
                        value=moisture,
                    )
                )
