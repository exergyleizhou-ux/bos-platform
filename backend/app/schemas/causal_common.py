"""Phase B — shared causal-layer schemas.

Hosts sub-models used by every ``/api/v1/causal/*`` endpoint:

- ``DagSpec`` (with acyclicity validator + smallest-cycle reporting)
- ``DagNode`` / ``DagEdge``
- ``CausalData`` (inline XOR server-side handle)
- ``IdentifiedEstimandHandle`` (echoed from /identify to /estimate)
- ``EstimateHandle`` (echoed from /estimate to /refute, /sensitivity)
- ``MethodParams`` (typed wrapper around per-method-family config)

Reference: PHASE_B_PLAN.md §2.6.
"""
...
class DagSpec(BaseModel):
    ...
    - no self-loops,
    - graph is acyclic; on violation the smallest cycle is reported
    ...
    def _check_dag_consistency(self) -> "DagSpec":
        ...
        # Acyclicity via NetworkX (already in BOS deps via dowhy).
...
class CausalData(BaseModel):
    ...
    Exactly one of ``inline`` or ``batch_query_key`` must be provided.
    ``fingerprint`` is mandatory and lets identify-verdicts be audited
    against the dataset they were issued for.
    """
...
class EstimateHandle(BaseModel):
    ...
    to re-fit on the same data + DAG + method choice. Stateless — no
    server-side cache is required.
    """
...
class DmlParams(BaseModel):
    ...
    Maps to ``econml.dml.LinearDML(model_y=..., model_t=..., ...)``.
    For Phase B MVP both nuisance models default to sklearn's
    ``LinearRegression``; alternatives are explicit strings, resolved
    by ``causal_estimate_engine``.
    """
...
class MethodParams(BaseModel):
    ...
    Each method_family populates exactly one sub-block; the others must
    be ``None``. Everything else is 422-rejected.
    """
