...
from app.deps import PaginationParams, get_current_user, get_pagination, require_role
from app.models import Batch, Calculation, Tenant, User
from app.schemas import MessageResponse, PaginatedResponse, TenantCreate, TenantResponse, TenantUpdate
...
async def get_tenant_stats(
    ...
    user_count = (await db.execute(select(func.count()).where(User.tenant_id == tenant_id))).scalar() or 0
    batch_count = (await db.execute(select(func.count()).where(Batch.tenant_id == tenant_id))).scalar() or 0
    calc_count = (await db.execute(select(func.count()).where(Calculation.tenant_id == tenant_id))).scalar() or 0
...
async def list_all_tenants(
    ...
    This is an admin-only platform management endpoint.
    """
    del current_user
    query = select(Tenant)
...
async def deactivate_tenant(
    ...
    """Deactivate a tenant without deleting its records."""
    del current_user
    ...
    return MessageResponse(message=f"Tenant '{tenant.slug}' deactivated")
