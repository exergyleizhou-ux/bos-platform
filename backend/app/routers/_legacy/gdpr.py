...
async def export_personal_data(
    ...
    batch_result = await db.execute(select(Batch).where(Batch.user_id == current_user.id).limit(10000))
    ...
    calc_result = await db.execute(select(Calculation).where(Calculation.user_id == current_user.id).limit(10000))
    ...
    # Audit logs
    audit_result = await db.execute(select(AuditLog).where(AuditLog.user_id == current_user.id).limit(5000))
...
async def request_erasure(
    ...
    await db.execute(Batch.__table__.update().where(Batch.user_id == current_user.id).values(operator="[REDACTED]"))
