...
from app.models import User, Tenant, Batch, Calculation, AuditLog
...
async def platform_stats(
    ...
    recent_batches = (await db.execute(
        select(func.count()).where(Batch.created_at >= yesterday)
    )).scalar() or 0
    recent_calcs = (await db.execute(
        select(func.count()).where(Calculation.created_at >= yesterday)
    )).scalar() or 0
    ...
    type_dist_result = await db.execute(
        select(Calculation.calc_type, func.count())
...
async def vacuum_database(
    ...
    tables = ["batches", "calculations", "audit_log"]
    results = {}
...
async def admin_users(
    ...
    """Admin-friendly user list used by the frontend."""
    result = await db.execute(
        select(User)
